...
static void pickRSEFRLGChunks(Language lang, const u8 **&chunkList, u32 &size)
{
    size = 1;
    switch(lang)
    {
...
static void __attribute__((noinline)) reinjectMysteryGiftPayloadTexts(u8 *section30Buffer, u8 *mgScriptBuffer, bool isFirstTime)
{
    ...
    const u8 **chunklist;
    u32 chunkListSize;
    Language lang = convertLangCodeIntoLanguage(curr_GBA_rom.language);

    pickRSEFRLGChunks(lang, chunklist, chunkListSize);
    FileContainerReader textReader(chunklist, chunkListSize, sizeof(decompressionBuffer));
    ...
    load_localized_charset(gen3Charset, 3, lang);
